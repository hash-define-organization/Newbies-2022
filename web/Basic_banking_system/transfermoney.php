...
<div class="main_div">
     ...
     <div class="container">
        <div class="row">
          ...
          <div class="col-sm-4">
              <div class="card text-center" style="margin-top:76px;background-color:Blue;border-radius:10px;color:white" >
              <form method="POST">  
                                              
 <?php
include 'connection.php';
$ids=$_GET['idtransfer'];
$showquery="select Name, Email from `users1` where ID=$ids";
...
?> 
          ...
          <div class="col-sm-4">
              <div class="card text-center" style="margin-top:60px;height:380px;">
                   <div class="card-body">
                   <img src="72.png" style="width:250px;height:220px;margin-top:40px;">
...
if(isset($_POST['submit']))
{
    ...
    $ids=$_GET['idtransfer'];
    $senderquery="select ID,Name, Email, Amount from `users1` where ID=$ids ";
    ...
    //receiverquery
    $receiverquery="select ID,Name,Email,Amount from `users1` where Email='$Receiver_email' ";
    ...
    if($arrdata['Amount'] >= $Sender)
    {
      ...
      $increase_receiver=$receiver_arr['Amount']+ $Sender;
       $query="UPDATE `users1` SET `ID`=$ids,`Amount`= $decrease_sender  where `ID`=$ids ";
       $rec_query="UPDATE `users1` SET `ID`=$id_receiver,`Amount`= $increase_receiver where  `ID`=$id_receiver ";
